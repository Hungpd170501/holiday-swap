...
public interface AvailableTimeRepository extends JpaRepository<AvailableTime, Long> {
    ...
    @Query(value = "select t.* from available_time t JOIN time_frame v on t.time_frame_id = v.time_frameID where v.property_id = ?1 AND v.room_id = ?2 AND ((?3 BETWEEN t.start_time AND t.end_time) OR ( ?4 BETWEEN t.start_time AND t.end_time)) Order By t.start_time ASC", nativeQuery = true)
